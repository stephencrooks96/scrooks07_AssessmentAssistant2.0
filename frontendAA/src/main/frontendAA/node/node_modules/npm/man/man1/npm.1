...
present, it is parsed\.  If the \fBuserconfig\fP optionContent is set in the cli
or env, then that will be used instead\.
.IP \(bu 2
Global Configs:
The file found at \.\./etc/npmrc (from the node executable, by default
this resolves to /usr/local/etc/npmrc) will be parsed if it is found\.
If the \fBglobalconfig\fP optionContent is set in the cli, env, or user config,
