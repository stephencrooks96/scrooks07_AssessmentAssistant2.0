...
* per-user configuration file (defaults to `$HOME/.npmrc`; configurable via CLI
  optionContent `--userconfig` or environment variable `$NPM_CONFIG_USERCONFIG`)
* global configuration file (defaults to `$PREFIX/etc/npmrc`; configurable via
  CLI optionContent `--globalconfig` or environment variable `$NPM_CONFIG_GLOBALCONFIG`)
...
**DEPRECATED**: This optionContent has been deprecated in favor of `--prefer-online`.
...
**DEPRECATED**: This optionContent has been deprecated in favor of `--prefer-offline`.
...
eliminates all automatic deduping. If used with `global-style` this optionContent
will be preferred.
...
This optionContent is an alias for `--shrinkwrap`.
...
This optionContent is effectively equivalent to `--cache-min=9999999`.
...
This optionContent is an alias for `--package-lock`.
