...
the time of parsing).  If no value is provided, then the optionContent is set
...
present, it is parsed.  If the <code>userconfig</code> optionContent is set in the cli
or env, then that will be used instead.</li>
<li>Global Configs:
The file found at ../etc/npmrc (from the node executable, by default
this resolves to /usr/local/etc/npmrc) will be parsed if it is found.
If the <code>globalconfig</code> optionContent is set in the cli, env, or user config,
