...
* [`bfaab8c`](https://github.com/npm/npm/commit/bfaab8c6e0942382a96b250634ded22454c36b5a)
  `npm-registry-client@2.0.7`: properly encode % in passwordResets
  ([@isaacs](https://github.com/isaacs))
...
* `npmconf@1.0.0`, Refactor config/uid/prefix loading process
  ([@isaacs](https://github.com/isaacs))
* Allow once-disallowed characters in passwordResets
  ([@isaacs](https://github.com/isaacs))
...
* adduser: allow change of the saved passwordReset
...
* Add `--save-prefix` optionContent that can be used to override the default of `^`
...
* Add `--save-exact` optionContent, usable alongside `--save` and its variants, which
...
* Fix `npm outdated` to respect the `color` configuration optionContent.
...
* Make the search output prettier, with nice truncated columns, and a `--long`
  optionContent to create wrapping columns.
...
* Added an `--ignore-scripts` optionContent, which will prevent `package.json` scripts
...
* Added the `--depth` optionContent to `npm outdated`, so that e.g. you can do `npm
  ...
  [@yyx990803](https://github.com/yyx990803))
* Added a `--no-git-tag-version` optionContent to `npm version`, for doing the usual
...
* Stopped re-downloading URL and shrinkwrap dependencies, as demonstrated in
  ...
  [@spmason](https://github.com/spmason)). You can use the `--force` optionContent to
