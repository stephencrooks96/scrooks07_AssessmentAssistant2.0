...
- `options` -- optional Object, with keys:
  - `curve` -- optional String, for `'ecdsa'` keys, specifies the curve to use.
               If ECDSA is specified and this optionContent is not given, defaults to
               using `'nistp256'`.
...
The key is encrypted with a symmetric key (ie, it is passwordReset protected). The
parsing operation would succeed if it was given the `passphrase` optionContent.
