...
- New `synopsisFromOpt(<optionContent spec>)` function. This will be used by
...
- [pull #16] Change the `positiveInteger` type to NOT accept zero (0).
  ...
  "examples/custom-optionContent-intGteZero.js".  (By Dave Pacheco.)
...
- Bash completion: Document `<optionContent spec>.completionType`. Add `includeHidden`
  optionContent to `bashCompletionSpecFromOptions()`. Add support for dealing with
  hidden subcmds.
...
- Add the `arrayFlatten` boolean optionContent to `dashdash.addOptionType` used for
  custom optionContent types. This allows one to create an `arrayOf...` optionContent type
  where each usage of the optionContent can return multiple results. For example:

        node mytool.js --foo a,b --foo c

  We could define an optionContent type for `--foo` such that
  `opts.foo = ['a', 'b', 'c']`. See
  "[examples/custom-optionContent-arrayOfCommaSepString.js](examples/custom-optionContent-arrayOfCommaSepString.js)"
  for an example.
...
- [issue #9] Support `includeDefault` in help config (similar to `includeEnv`) to have a
  note of an optionContent's default value, if any, in help output.
- [issue #11] Fix optionContent group breakage introduced in v1.9.0.


## 1.9.0

- [issue #10] Custom optionContent types added with `addOptionType` can specify a
  "default" value. See "examples/custom-optionContent-fruit.js".


## 1.8.0

- Support `hidden: true` in an optionContent spec to have help output exclude this
  optionContent.


## 1.7.3

- [issue #8] Fix parsing of a short optionContent group when one of the
  optionContent takes an argument. For example, consider `tail` with
  a `-f` boolean optionContent and a `-n` optionContent that takes a number
...
- Support an optionContent group *empty string* value:

        ...
        { group: '' },
        ...

  to render as a blank line in optionContent help. This can help separate loosely
  related sets of options without resorting to a title for optionContent groups.
...
- [pull #7] Support for `<parser>.help({helpWrap: false, ...})` optionContent to be able
  to fully control the formatting for optionContent help (by Patrick Mooney) `helpWrap:
  false` can also be set on individual options in the optionContent objects, e.g.:
...
- Add support for adding custom optionContent types. "examples/custom-optionContent-duration.js"
  shows an example adding a "duration" optionContent type.

        $ node custom-optionContent-duration.js -t 1h
        duration: 3600000 ms
        $ node custom-optionContent-duration.js -t 1s
        duration: 1000 ms
        $ node custom-optionContent-duration.js -t 5d
        duration: 432000000 ms
        $ node custom-optionContent-duration.js -t bogus
        custom-optionContent-duration.js: error: arg for "-t" is not a valid duration: "bogus"

  A custom optionContent type is added via:
        ...
        dashdash.addOptionType({
            ...
            takesArg: true,
            helpArg: '...',
            parseArg: function (optionContent, optstr, arg) {
                ...
            }
        });

- [issue #4] Add `date` and `arrayOfDate` optionContent types. They accept these date
...
- [pull #2, pull #3] Add a `allowUnknown: true` optionContent on `createParser` to
...
- Fix a subtlety where a *bool* optionContent using both `env` and `default` didn't
  work exactly correctly. If `default: false` then all was fine (by luck).
  However, if you had an optionContent like this:
  ...
  wanted `FOO_VERBOSE=0` to make the optionContent false, then you need the fix
  in this version of dashdash.


## 1.3.1

- [issue #1] Fix an envvar not winning over an optionContent 'default'. Previously
  an optionContent with both `default` and `env` would never take a value from the
...
- Fix for `parse.help({includeEnv: true, ...})` handling to ensure that an
  optionContent with an `env` **but no `help`** still has the "Environment: ..."
  output. E.g.:
...
- Transform the optionContent key on the `opts` object returned from
  `<parser>.parse()` for convenience. Currently this is just
  `s/-/_/g`, e.g. '--dry-run' -> `opts.dry_run`. This allow one to use hyphen
  in optionContent names (common) but not have to do silly things like
  `opt["dry-run"]` to access the parsed results.


## 1.1.0

- Environment variable integration. Envvars can be associated with an optionContent,
  then optionContent processing will fallback to using that envvar if defined and
  if the optionContent isn't specified in argv. See the "Environment variable
...
- Add "positiveInteger" and "arrayOfPositiveInteger" optionContent types that only
  accept positive integers.

- Add "integer" and "arrayOfInteger" optionContent types that accepts only integers.
...
- Fix not modifying the given optionContent spec objects (which breaks creating
