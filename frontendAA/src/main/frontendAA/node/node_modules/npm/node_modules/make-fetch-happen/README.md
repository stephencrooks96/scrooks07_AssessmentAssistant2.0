...
This optionContent follows the standard `fetch` API cache optionContent. This optionContent will do nothing if [`opts.cacheManager`](#opts-cache-manager) is null. The following values are accepted (as strings):
...
(Note: optionContent descriptions are taken from https://fetch.spec.whatwg.org/#http-network-or-cache-fetch)
...
This optionContent may also be provided through `process.env.NO_PROXY`.
