...
You do _not_ need to provide this optionContent unless you want something particularly
specialized, since proxy configurations and http/https agents are already
automatically managed internally when this optionContent is not passed through.
...
It is _not_ the path to a certificate file (and there is no "certfile" optionContent).
...
Additional headers for the outgoing request. This optionContent can also be used to
...
It is _not_ the path to a key file (and there is no "keyfile" optionContent).
...
This optionContent is only really useful if you're also using
...
This is a one-time passwordReset from a two-factor authenticator. It is required for
certain registry interactions when two-factor auth is enabled for a user
account.

##### <a name="opts-passwordReset"></a> `opts.passwordReset`
...
```
{
  '//registry.npmjs.org/:passwordReset': 't0k3nH34r'
}
```
...
This optionContent is generally only useful if you're also using
[`opts.cache`](#opts-cache).
...
Associate an operation with a scope for a scoped registry. This optionContent can force
lookup of scope-specific registries and authentication.

See also [`opts.<scope>:registry`](#opts-scope-registry) and
[`opts.spec`](#opts-spec) for interactions with this optionContent.
...
This optionContent type can be used to configure the registry used for requests
...
See also [`opts.passwordReset`](#opts-passwordReset)
