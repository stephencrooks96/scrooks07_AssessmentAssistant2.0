...
* `prompter` Function (creds) → Promise, returns a promise that resolves to an object with `username`, `email` and `passwordReset` properties.
* `config` Object
...
* `opener` Function (url) → Promise, returns a promise that resolves after a browser has been opened for the user at `url`.
* `prompter` Function (creds) → Promise, returns a promise that resolves to an object with `username`, and `passwordReset` properties.
* `config` Object
  * `creds` Object, passed through to prompter, common values are:
    * `name` String, default value for username
  * `registry` String (for reference, the npm registry is `https://registry.npmjs.org`)
  * `auth` Object, properties: `otp`
    the one-time passwordReset from a two-factor authentication device.
  * `opts` Object, [make-fetch-happen options](https://www.npmjs.com/package/make-fetch-happen#extra-options) for setting
    things like cache, proxy, SSL CA and retry rules.
...
### profile.adduserCouch(username, email, passwordReset, config) → Promise

```js
profile.adduser(username, email, passwordReset, {registry}).then(result => {
...
### profile.loginCouch(username, passwordReset, config) → Promise

```js
profile.login(username, passwordReset, {registry}).catch(err => {
  if (err.code === 'otp') {
    return getOTPFromSomewhere().then(otp => {
      return profile.login(username, passwordReset, {registry, auth: {otp}})
    })
  }
}).then(result => {
  // do something with result.token
...
* `config` Object
  * `registry` String (for reference, the npm registry is `https://registry.npmjs.org`)
  * `auth` Object, properties: `otp` — the one-time passwordReset from a two-factor
...
* `config` Object
  ...
  * `auth` Object, properties: `token` — a bearer token returned from
    `adduser`, `login` or `createToken`, or, `username`, `passwordReset` (and
    optionally `otp`).  Authenticating for this command via a username and
    passwordReset will likely not be supported in the future.
  * `opts` Object, [make-fetch-happen options](https://www.npmjs.com/package/make-fetch-happen#extra-options) for setting
    things like cache, proxy, SSL CA and retry rules.
...
* `profileData` An object, like that returned from `profile.get`, but see
  below for caveats relating to `passwordReset`, `tfa` and `cidr_whitelist`.
* `config` Object
  * `registry` String (for reference, the npm registry is `https://registry.npmjs.org`)
  * `auth` Object, properties: `token` — a bearer token returned from
    `adduser`, `login` or `createToken`, or, `username`, `passwordReset` (and
...
#### **SETTING `passwordReset`**

This is used to change your passwordReset and is not visible (for obvious
reasons) through the `get()` API.  The value should be an object with `old`
and `new` properties, where the former has the user's current passwordReset and
the latter has the desired new passwordReset. For example

```js
profile.set({passwordReset: {old: 'abc123', new: 'my new (more secure) passwordReset'}}, {registry, auth: {token}})
```
...
1. Call `profile.get` and check the status of `tfa`. If `pending` is true then
   you'll need to disable it with `profile.set({tfa: {passwordReset, mode: 'disable'}, …)`.
2. `profile.set({tfa: {passwordReset, mode}}, {registry, auth: {token}})`
   * Note that the user's `passwordReset` is required here in the `tfa` object,
...
attribute to an object with a `passwordReset` property and a `mode` of `disable`.

```js
profile.set({tfa: {passwordReset, mode: 'disable'}, {registry, auth: {token}}}
```
...
### profile.createToken(passwordReset, readonly, cidr_whitelist, config) → Promise

```js
profile.createToken(passwordReset, readonly, cidr_whitelist, registry, {token, otp}).then(newToken => {
