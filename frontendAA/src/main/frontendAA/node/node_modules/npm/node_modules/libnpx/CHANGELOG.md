...
* **npm:** Use --parseable optionContent to work around output quirks ([8cb75a2](https://github.com/zkat/npx/commit/8cb75a2))
...
for npx execution, with no `-p` optionContent or `-c`. This might cause npx to
...
* **ignore:** add --ignore-existing optionContent (#20) ([0866a83](https://github.com/zkat/npx/commit/0866a83))
* **install:** added --no-install optionContent to prevent install fallbacks ([a5fbdaf](https://github.com/zkat/npx/commit/a5fbdaf))
...
* **commands:** If a command has an explicit --package optionContent, or if the command has an @version part, any version of the command in $PATH will be ignored and a regular install will be executed.
