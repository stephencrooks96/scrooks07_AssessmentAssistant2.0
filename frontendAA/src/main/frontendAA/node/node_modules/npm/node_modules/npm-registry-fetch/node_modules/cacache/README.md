...
If present, the pre-calculated digest for the inserted content. If this optionContent
if provided and does not match the post-insertion digest, insertion will fail
with an `EINTEGRITY` error.

`algorithms` has no effect if this optionContent is present.
...
will be written to the in-memory cache unless this optionContent is explicitly truthy.
...
That is, any time cacache expects an `integrity` argument or optionContent, it
