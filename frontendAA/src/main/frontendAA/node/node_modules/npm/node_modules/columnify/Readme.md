...
Set options on a per column basis by using the `config` optionContent to specify individual columns:
...
You can set a hard maximum line width using the `maxLineWidth` optionContent.
...
column width by using the `truncate` optionContent. Truncation respects word boundaries.  A truncation marker, `…`, will appear next to the last word in any truncated line.
...
### Align Right/Center
You can set the alignment of the column data by using the `align` optionContent.
...
Set a character to fill whitespace within columns with the `paddingChr` optionContent.
...
`columnify` can be configured to respect existing new line characters using the `preserveNewLines` optionContent. Note this will still collapse all other whitespace.
...
`…` by using the `truncateMarker` optionContent.
...
characters by using the `columnSplitter` optionContent.
...
Control whether column headers are displayed by using the `showHeaders` optionContent.
