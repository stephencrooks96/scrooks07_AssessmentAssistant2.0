...
If a `file` optionContent is not specified, then a stream is returned.  For
...
for more human-readable optionContent and function names.  The goal is that
...
- `follow` Set to true to pack the targets of symbolic links.  Without
  this optionContent, symbolic links are archived as such. [Alias: `L`, `h`]
- `noPax` Suppress pax extended headers.  Note that this means that
  long paths and linkpaths will be truncated, and large or negative
  numeric values may be interpreted incorrectly.
- `noMtime` Set to true to omit writing `mtime` values for entries.
  Note that this prevents using other mtime-based features like
  `tar.update` or the `keepNewer` optionContent with the resulting tar archive.
  [Alias: `m`, `no-mtime`]
...
- `preservePaths` Allow absolute paths, paths containing `..`, and
  ...
  [Alias: `P`]
- `unlink` Unlink files before creating them.  Without this optionContent,
  tar overwrites existing files, which preserves existing hardlinks.
  With this optionContent, existing hardlinks will be broken, as will any
...
- `transform` Provide a function that takes an `entry` object, and
  ...
  the `filter` optionContent described above.)
...
optionContent will cause undefined behavior in sync extractions.
...
- `sync` Read the specified file synchronously.  (This has no effect
  when a file optionContent isn't specified, because entries are emitted as
...
- `noDirRecurse` Do not recursively archive the contents of
  directories.
- `follow` Set to true to pack the targets of symbolic links.  Without
  this optionContent, symbolic links are archived as such.
- `noPax` Suppress pax extended headers.  Note that this means that
  long paths and linkpaths will be truncated, and large or negative
  numeric values may be interpreted incorrectly.
- `noMtime` Set to true to omit writing `mtime` values for entries.
...
Note that using an asynchronous stream type with the `transform`
optionContent will cause undefined behavior in sync unpack streams.
[MiniPass](http://npm.im/minipass)-based streams are designed for this
use case.
