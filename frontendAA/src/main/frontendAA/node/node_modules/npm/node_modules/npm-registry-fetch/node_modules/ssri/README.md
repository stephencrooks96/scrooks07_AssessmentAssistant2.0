...
* `?foo` metadata optionContent support.
...
The `opts.sep` optionContent defines the string to use when joining multiple entries
...
`opts.options` may optionally be passed in: it must be an array of optionContent
