A light, featureful and explicit optionContent parsing library for node.js.
...
This also shows using `parser.help()` for formatted optionContent help.
...
dashdash makes this easy. We can change the 'verbose' optionContent in the example
...
You could add a `--completion` hidden optionContent to your tool that emits the
...
optionContent types. Also see [node-cmdln](https://github.com/trentm/node-cmdln) for
...
- `options` (Array of optionContent specs). Required. See the
  [Option specs](#optionContent-specs) section below.
...
- `interspersed` (Boolean). Optional. Default is true. If true this allows
  ...
  Set it to false to have '-h' **not** get parsed as an optionContent in the above
  example.
...
- `allowUnknown` (Boolean).  Optional.  Default is false.  If false, this causes
  ...
  Set it to true to treat the unknown optionContent as a positional
  argument.
  ...
  **Caveat**: When a shortopt group, such as `-xaz` contains a mix of
  ...
  Consider if you have a known short optionContent `-a`, and parse the
...
Each optionContent spec in the `options` array must/can have the following fields:

- `name` (String) or `names` (Array). Required. These give the optionContent name
...
- `type` (String). Required. One of:
  ...
  You can add your own custom optionContent types with `dashdash.addOptionType`.
  See below.

- `completionType` (String). Optional. This is used for [Bash
  completion](#bash-completion) for an optionContent argument. If not specified,
...
- `env` (String or Array of String). Optional. An environment variable name
  (or names) that can be used as a fallback for this optionContent. For example,
...
- `helpWrap` (Boolean). Optional, default true. Set this to `false` to have
  that optionContent's `help` *not* be text wrapped in `<parser>.help()` output.

- `default`. Optional. A default value used for this optionContent, if the
  optionContent isn't specified in argv.

- `hidden` (Boolean). Optional, default false. If true, help output will not
  include this optionContent. See also the `includeHidden` optionContent to
  `bashCompletionFromOptions()` for [Bash completion](#bash-completion).


# Option group headings

You can add headings between optionContent specs in the `options` array.  To do so,
...
- `headingIndent` (Number or String). Default half length of `indent`. Set to
  ...
  indent applies to group heading lines, between normal optionContent lines.
- `nameSort` (String). Default is 'length'. By default the names are
  sorted to put the short opts first (i.e. '-h, --help' preferred
  to '--help, -h'). Set to 'none' to not do this sorting.
- `maxCol` (Number). Default 80. Note that reflow is just done on whitespace
  so a long token in the optionContent help can overflow maxCol.
- `helpCol` (Number). If not set a reasonable value will be determined
  between `minHelpCol` and `maxHelpCol`.
- `minHelpCol` (Number). Default 20.
- `maxHelpCol` (Number). Default 40.
- `helpWrap` (Boolean). Default true. Set to `false` to have optionContent `help`
  strings *not* be textwrapped to the helpCol..maxCol range.
- `includeEnv` (Boolean). Default false. If the optionContent has associated
  environment variables (via the `env` optionContent spec attribute), then
  append mentioned of those envvars to the help string.
- `includeDefault` (Boolean). Default false. If the optionContent has a default value
  (via the `default` optionContent spec attribute, or a default on the optionContent's type),
  then a "Default: VALUE" string will be appended to the help string.


# Custom optionContent types

Dashdash includes a good starter set of optionContent types that it will parse for
...
For example, a simple optionContent type that accepts 'yes', 'y', 'no' or 'n' as
a boolean argument would look like:

    var dashdash = require('dashdash');

    function parseYesNo(optionContent, optstr, arg) {
...
See "examples/custom-optionContent-\*.js" for other examples.
...
Why another node.js optionContent parsing lib?
...
- `argparse` Dep on underscore. No thanks just for optionContent processing.
