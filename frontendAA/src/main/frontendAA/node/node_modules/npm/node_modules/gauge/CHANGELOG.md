...
* Default to `enabled` only if we have a tty.  Users can always override
  this by passing in the `enabled` optionContent explicitly or by calling calling
  `gauge.enable()`.
...
* Previously the gauge was disabled by default if `process.stdout` wasn't a
  ...
  behavior set the `enabled` optionContent to `process.stdout.isTTY`.
